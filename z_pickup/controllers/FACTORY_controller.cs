...
public class FactoryController : ControllerBase {
    ...
    public async Task<IActionResult> Get(string userId, string apiKey) {
        ...
        string filePath = Path.Combine(Directory.GetCurrentDirectory(), GLOBAL.pickupFactoryDirectory);
        ...
        if (!System.IO.File.Exists(filePath)) {
            ...
            model.DevMessage = $"file factory not found in server db";
            return Ok(model);
        }
        ...
        List<FactoryModel>? data;
        try {
            data = JsonSerializer.Deserialize<List<FactoryModel>>(jsonString);
