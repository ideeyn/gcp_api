...
public class AgentController : ControllerBase {
    ...
    public async Task<IActionResult> Get(string userId, string apiKey) {
        ...
        string filePath = Path.Combine(Directory.GetCurrentDirectory(), GLOBAL.pickupAgentDirectory);
        ...
        if (!System.IO.File.Exists(filePath)) {
            ...
            model.DevMessage = $"file agent not found in server db";
            return Ok(model);
        }
        ...
        try {
            data = JsonSerializer.Deserialize<List<AgentModel>>(jsonString);
