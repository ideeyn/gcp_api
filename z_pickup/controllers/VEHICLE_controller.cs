...
public class VehicleController : ControllerBase {
    ...
    public async Task<IActionResult> Get(string userId, string apiKey) {
        ...
        string filePath = Path.Combine(Directory.GetCurrentDirectory(), GLOBAL.pickupVehicleDirectory);
        ...
        if (!System.IO.File.Exists(filePath)) {
            ...
            model.DevMessage = $"file vehicle not found in server db";
            return Ok(model);
        }
        ...
        try {
            data = JsonSerializer.Deserialize<List<VehicleModel>>(jsonString);
