...
public class ZoneController : ControllerBase {
    ...
    public async Task<IActionResult> Get(string userId, string apiKey) {
        ...
        string filePath = Path.Combine(Directory.GetCurrentDirectory(), GLOBAL.pickupZoneDirectory);
        ...
        if (!System.IO.File.Exists(filePath)) {
            ...
            model.DevMessage = $"file zone not found in server db";
            return Ok(model);
        }
        ...
        List<ZoneModel>? data;
        try {
            data = JsonSerializer.Deserialize<List<ZoneModel>>(jsonString);
