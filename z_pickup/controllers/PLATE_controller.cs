...
public class PlateController : ControllerBase {
    ...
    public async Task<IActionResult> Get(string userId, string apiKey) {
        ...
        string filePath = Path.Combine(Directory.GetCurrentDirectory(), GLOBAL.pickupPlateDirectory);
        ...
        if (!System.IO.File.Exists(filePath)) {
            ...
            model.DevMessage = $"file plate not found in server db";
            return Ok(model);
        }
        ...
        try {
            data = JsonSerializer.Deserialize<List<PlateModel>>(jsonString);
